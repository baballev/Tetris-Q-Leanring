...
class TetrisTrainer():
    ...
    def __init__(self, capacity=500000, batch_size=128, gamma=0.999, lr=0.0001, epsilon_start=0.99, schedule_size=1000000):
        ...
        self.q_network = model.TetrisNetwork(input_dim=200, hidden_size=32, output_dim=5)  # The one we choose the actions from
        self.target_q_network = model.TetrisNetwork(input_dim=200, hidden_size=32, output_dim=5)  # The one we will be optimizing from (in part)
        self.update_target()
    ...
    def optimize(self):
        ...
        state, new_state, action, reward = batch[:, 0], batch[:, 1], batch[:, 2], batch[:, 3]
